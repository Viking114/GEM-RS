...
class GloVeModel():
    ...
    def _similarity_query(self, word_vec, number):
        ...
        vec = self.embeddings
        dst = (np.dot(vec, word_vec)
               / np.linalg.norm(vec, axis=1)
               / np.linalg.norm(word_vec))
        word_ids = np.argsort(-dst)

        inverse_dict = {v: k for k, v in self.__word_to_id.items()}

        return [(inverse_dict[x], dst[x]) for x in word_ids[:number]
                if x in inverse_dict]
    ...
    def most_similar(self, word, number=5):
        ...
        """
        try:
            word_idx = self.__word_to_id[word]
        except KeyError:
            raise Exception('Word not in dictionary')

        vec = self.embeddings

        return self._similarity_query(vec[word_idx], number)[1:]
